...
INSTALL_DIR=$PWD/../opencore-amr_${TARGET_ARCH_ABI}_install

export LD="$ARM_TOOL/bin/$abi-ld"
export CC="$ARM_TOOL/bin/$abi-gcc"
export CXX="$ARM_TOOL/bin/$abi-g++"
#Export FLAGS from ndk-build instead
...
export CFLAGS="$LOCAL_CFLAGS -I$ARM_INC"
...
export CXXFLAGS="$LOCAL_CXXFLAGS -I$ARM_INC"
...
export LDFLAGS="$LOCAL_LDFLAGS -L$ARM_LIBO -nostdlib -Bdynamic  -Wl,--no-undefined -Wl,-z,noexecstack  -Wl,-z,nocopyreloc -Wl,-soname,/system/lib/libz.so -Wl,-rpath-link=$PLATFORM/usr/lib,-dynamic-linker=/system/bin/linker -L$ARM_LIB  -lc -lm -ldl -lsupc++ -Wl,--library-path=$PLATFORM/usr/lib/ -Xlinker $PLATFORM/usr/lib/crtbegin_dynamic.o -Xlinker $PLATFORM/usr/lib/crtend_android.o "

./configure --prefix=$INSTALL_DIR --host=arm || exit -1
